...
public class SaleOrderClient {
	...
	private static final String BASE_URL = "http://localhost:8080/1/sale-order";
	...
	public static SaleOrderDto save(SaleOrderDto saleOrderDto) {
		try {
			final HttpResponse<String> httpResponse = Unirest.post(BASE_URL).body(saleOrderDto).asString();
			...
			return new Gson().fromJson(httpResponse.getBody(), SaleOrderDto.class);
	...
	public static SaleOrderDto getById(Long id) {
		try {
			...
			final Type type = new TypeToken<ResponseDto<SaleOrderDto>>() {
			}.getType();

			final ResponseDto<SaleOrderDto> responseDto = new Gson().fromJson(httpResponse.getBody(), type);
	...
	public static List list() {
		try {
			...
			final Type listType = new TypeToken<List<SaleOrderDto>>() {
			}.getType();
